...
app.post('/purchase', (req, res) => {
    ...
    // check if bookID is one of the books in validBooks
    if (!validBooks.find(book => book.id == bookID)) {
        res.send('Invalid book ID');
    } else {
        // modify session to include book
...
app.post('/download-ebook', (req, res) => {
    ...
    switch (option) {
        case 'direct':
            // send book
            res.send('Direct downloads currently unavailable. Please wait until the established publish date!');
            break;
        case 'kindle':
